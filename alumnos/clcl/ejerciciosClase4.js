  ...
  log() {
    return this.libros.map((libro) => {
      return {
        ...
        autor: libro.autor,
        leido: libro.leido
          ? `Ya has leído ${libro.nombre}`
          : `Aun no has leído ${libro.nombre}`,
      };
    });
  },
  sugerencia() {
    const arrayLibrosNoLeidos = this.libros
    ...
    const indexRandom =
      Math.floor(Math.random() * (arrayLibrosNoLeidos.length - 0)) + 0;
    return arrayLibrosNoLeidos[indexRandom];
  },
};
